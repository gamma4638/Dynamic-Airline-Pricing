...
Examples:
  ...
  python driver.py -p fixed --fixed-price 12            # Fixed Price 12(십만원)로 설정
    ...
    # - 높은 가격대, 낮은 가격 탄력성(-0.6), 낮은 구매 포기율(5%)
    model_params1 = {
        'num_seats': 1200,
        'num_stages': 90,
        'price_min': 0.0,      # 단위: 십만원
        'price_max': 30.0,
        'price_step': 0.1,
    ...
    # - 낮은 가격대, 높은 가격 탄력성(-1.2), 높은 구매 포기율(10%)
    model_params2 = {
        'num_seats': 600,
        'num_stages': 90,
        'price_min': 0.0,       # 단위: 십만원
        ...
        'demand_beta': 0.03,       # 지수 수요 모델: 감쇠율
        'v_me': 9.05,           # V₁: 자사 효용
        'v_comp': 8.16,         # V₂: 경쟁사 효용
        'v_no_buy': 0.0,        # V₀: 구매 포기 효용 (기준점)
        'beta': 1.2,          # 가격 민감도
        'p_comp_min': 6.0,      # 경쟁사 최소 가격 (t=0)
        'p_comp_max': 18.0,     # 경쟁사 최대 가격 (t=T-1)
    }
    ...
    int_time = int(time.time()) % (2 ** 32)
    np.random.seed(int_time)
    number = np.random.randint(1000000, 9999999)
    number = str(number)
    ...
    if args.mode in ['single', 'both']:
        ...
        file_name = f"{number}_episode_result.png"
    ...
    if args.policy == 'dp':
        ...
        file_name1 = f"{number}_policy_3d.png"
        file_name2 = f"{number}_policy_heatmap.png"
        plot_policy_3d(
            policy.policy_table,
            title=f"Optimal Policy 3D ({policy_name})",
            save_path=os.path.join(fig_dir, file_name1)
        )
        plot_policy_heatmap(
            policy.policy_table,
            title=f"Optimal Policy Heatmap ({policy_name})",
            save_path=os.path.join(fig_dir, file_name2)
        )
