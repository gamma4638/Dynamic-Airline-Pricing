...
class AirlinePricingModel:
    ...
    def __init__(
        ...
        v_no_buy: float = 1.0,
        beta: float = 0.01,
        p_comp_min: float = 8.0,
        p_comp_max: float = 18.0,
    ):
        """
        Args:
    ...
    def get_competitor_price(self, t: int, rng: np.random.Generator = None) -> float:
        """
        시간 t에 따른 경쟁사 가격 반환 (지수 함수 형태)
        
        수식: P(t) = min * (max/min)^(t / (T-1))
        - t=0일 때: p_comp_min
        - t=T-1일 때: p_comp_max
        - 그 사이: 지수적으로 증가 (J-Curve)
        ...
        Returns:
            경쟁사 가격
        """
        # 마지막 시점 인덱스 (총 기간이 90일이면 t=89가 마지막)
        t_max_index = max(1, self.num_stages - 1)
        
        # 지수 증가 (Geometric Growth) 계산
        ratio = self.p_comp_max / self.p_comp_min
        base = self.p_comp_min * (ratio ** (t / t_max_index))
